...
// Defining callback fucntions for the trackbars to update parameters values
...
static void on_tranckbar2(int, void*)
{
...
static void on_tracker8(int, void*)
{
  stereo->setSpeckleRange(speckleRange);
}

static void on_tracker9(int, void*)
{
  stereo->setSpeckleWindowSize(speckleWindowSize*2);
  speckleWindowSize = speckleWindowSize*2;
}

static void on_tracker10(int, void*)
{
  stereo->setDisp12MaxDiff(disp12MaxDiff);
}

static void on_tracker11(int, void*)
{
...
int main()
{
  // Initialize variable to store the maps for stereo rectification
  cv::Mat Left_Stereo_Map1, Left_Stereo_Map2, Right_Stereo_Map1, Right_Stereo_Map2;

  // Reading the mapping values for stereo image rectification
  cv::FileStorage cv_file2 = cv::FileStorage("data/stereo_rectify_maps.xml",
                                             cv::FileStorage::READ);
  ...
  cv::VideoCapture camL(CamL_id);
  cv::VideoCapture camR(camR_id);

  // Check if left camera is attached
  if(!camL.isOpened())
  {
    std::cout<<"Could not open camera with index: " 
             <<CamL_id
             <<std::endl;
    return -1;
  }
  ...
  // Creating a nmed window to be linked to the trackbars
  cv::namedWindow("disparity", cv::WINDOW_NORMAL);
  cv::resizeWindow("disparity", 1920, 1080);
  ...
  while(true)
  {
    ...
    // NOTE: Code returns a 16 bits signed single channel image,
    // CV_16S containing s disparity map scaled by 16. Hence it
